...
### Initial user experience / Main Screen default state
User sees the app title and a scrollable list of the latest alert titles. Each alert title is clickable and it takes the user to an alert details view.
There is a button to customize the alert list on the bottom of the screen.

### The alert details view
Alert title on top with bold
Alert details in a scrollable frame
Alert tags and link on the bottom
The link open in the users' default browser and navigates to the FDA / USDA web page with the original alert
Up / Down buttons on top to navigate between individual alerts; swipe up / down should also navigate between alerts
Swipe back should go back to the previous screen

### The Customize alerts view
The user can specify the location of the alert (where  the alert is effective), the date range (eg from the last n weeks, keep it simple) as well as keywords to alert for. This in turn generates a query that executes on the openFDA and USDA APIs.

The generated query can be saved with a name on the bottom of the screen. It saves into the phone's default Settings storage and serializes to the user's backup (iCloud, Google Drive) similarly to system info, like contacts or notes.

## Architecture
This is a client phone app.

It saves the queries in the phone preferences storage.
...
This should be backed up to iCloud
...
## Stack:
Phone app using a web stack for UI and logic
...
then:
cd food-safety-recall-alert-app
pnpm install
